...
void searchParameters(AssemblyParser &parser, const bool &useBin, MMIO &mmio){
    ...
    for(uint32_t way: wayList){
        for(uint32_t tag = tagMin; tag < (REGISTER_BIT_N - offsetMin -1); tag++){
...
// パラメタの下限
    ...
    for(int i = 0; i < paramN; i++){
        std::cout << "tag数: " << tagMin << " way数: " << ways[i] << " offset数: " << offsets[i] << std::endl;
        
        AssemblySimulator simulator(parser, useBin, false, mmio, ways[i], offsets[i], tagMin);
        simulator.launch(false);
        if(first){
            first = false;
            accessN[Cache::READ] = simulator.cache.hitN[Cache::READ] + simulator.cache.initMissN[Cache::READ] + simulator.cache.otherMissN[Cache::READ];
        ...
        for(int j = 0; j < Cache::TYPES_N; j++){
            ...
            std::cout << " ヒット数:" << hitN << ", ミス率: " << (accessN[j] - hitN) / (double)accessN[j] << std::endl;
