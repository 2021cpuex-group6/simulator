...
# 1stコアシミュレータver2.9, アセンブラ、FPUシミュ，変換ツール
...
実行時オプションの説明
- `-b` ... バイナリファイルで実行できます
- `-p_[way]_[offset]_[tag]` ... キャッシュのウェイ数，オフセット長，タグ長を指定して実行できます． 
- `-d_[path]` ... uart通信でコアに送信されるデータファイル(sldファイル)を指定します．
...
- `lr [printN]`  ... (label ranking) ジャンプ先となったラベルのうち上位`[printN]`個を多い順に表示
...
- `fr`  ... (floating table ranking) ... 浮動小数点テーブルをアクセス数順に表示する
...
- `--byte` ... (byte) バイトごとにテキスト形式で出力します．
- `--word` ... (word) ワードごとにテキスト形式で出力します．
以上で，実行すると，`./test/`内の`.s`ファイルが`.s.out`ファイルとして全てコンパイルされます．
(`--byte`, `--word`を指定した場合は，それぞれ`.s.outb`, `.s.outw`という拡張子になります．)
...
### デバッグモード
`--debug`をつけて起動することでデバッグモードにできます(シミュ係用)．
以下使えるコマンド
- `pp` ... (program's profile) 時間予測に必要なプログラムのデータをファイル出力
    - profフォルダ以下に各プログラムごとにフォルダを分けてファイル出力される．
    - `--time`をつけて起動することでこれに基づいて時間を計算するだけのモードになる．
...
- 2.30 ... バグ修正，MMIO機能を追加
- 2.40 ... バグ修正，機械語のバイナリに対応，ブレークポイント機能の復活（仕様変更）
- 2.50 ... バグ修正，MMIOの情報を見ることを可能に，GUI用の機能を追加
- 2.60 ... バグ修正，fdivの仕様変更
- 2.70 ... バグ修正など，これで1st完動
- 2.80 ... `lr`を実装
- 2.90 ... バグ修正，時間予測が完成